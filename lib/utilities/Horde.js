...
class Horde {
    ...
    setLimit (limit) {
        ...
        if (typeof limit !== 'number' || isNaN(limit)) throw new SyntaxError('Limit must be a number.')
        if (limit < 0) throw new RangeError('Limit must be above 0.')
        this.hordeLimit = l
        return limit
    }
    ...
    set (key, value) {
        ...
        if (this.size === this.limit) throw new Error('Horde has exceeded maximum size.')
